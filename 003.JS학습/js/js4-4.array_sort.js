...
function sortingsFn(evt, cta, arrData, exBox) {
  ...
  // 일반적인 값의 배열 깊은 복사는 새변수 = [...원본배열변수] -> 이렇게 하지만
...
function searchingFn() {
  ...
  // 전체물자열.indexOf(문자열) -> 해당문자열이 전체문자열에서
...
// -> 실제 정렬을 적용하여 리스트를 갱신한다
// -> 정렬 적용시 정렬기준 대상 선택항목을 가져가야함
mFn.addEvt(sel4, "change", (e) =>
  sortingsFn(e, cta4.value, searchResult ? searchResult : list2, showList4)
);
...
// -> 실제 정렬을 적용하여 리스트를 갱신한다
// -> 정렬 적용시 정렬기준 대상 선택항목을 가져가야함
mFn.addEvt(sel5, "change", (e) =>
  sortingsFn(e, cta5.value, arrList3, showList5)

...
// 메인 출력 컴포넌트 ////
// 컴포넌트 내부에서 호출하는 다른 컴포넌트를 보통 서브컴포넌트라고 부른다
function Developer({ isNormal, isrc, ialt, itit }) {
  ...
  // 위의 if문에 return에 걸리면 아랫쪽 return에는 내려오지 않는다
...
/********************************************************************************** 
    ...
    -> 조건식은 true / false 의 결과가 나오는 식이며 AND 조건, OR 조건, NOT조건식이 모두 사용가능함
    예) (A == "사자" && (B > 10 || C == "별" || !D))
**********************************************************************************/
...
function MakeList({ foodName, movieInfo }) {
  ...
  return (
    <li>
      {
        // 음식 데이터가 들어온 경우
        // undefined가 아니면 true 이므로 && 뒤 코드 출력
...
function WishList({ wList }) {
  ...
  return (
    <React.Fragment>
          ...
          <ul>
            {
              ...
              // 그러나 리액트는 이것을 변경하여 표현식안에서 출력시 태그JSX 형식으로 변환해 준다. JS처럼 map().join('')처리 불필요
            }
          </ul>
        </div>
      )}
      {/* 배열값이 0인 경우 다른것 출력하기 */}
      {wList.length == 0 && (
        <h2>아직 개발자 음식 리스트가 업데이트 되지 않음</h2>
      )}
    </React.Fragment>
  );
} ////// WishList 컴포넌트 ////////
...
// 특이사항 : 변경버튼 클릭시 MakePainting 컴포넌트의 데이터를 변경하여 다시 출력하도록 함 (Hook사용)
  ...
  return (
    <React.Fragment>
      ...
      <button
        ...
        style={{ fontSize: "30px", padding: "10px", fontWeight: "bold", backgroundColor:result?"red":"blue",color:result?"yellow":"aqua" }}
      >
        작가변경
      </button>
      {/* 3. 작품출력 */}

...
function goGame() {
  ...
  if (btxt === "토끼출발") {
    ...
    // 거북버튼 클릭후 포커스로 인해 엔터버튼을 사용할수 있으므로 이를 막기위해 포커스해제
    // 즉, blur() 메서드로 처리함
    this.blur();
...
let autoI;
function goR1() {
  // 호출이 한번만 되도록 autoI가 할당전엔 undefined 이므로 if문에서 false 처리됨 이것 이영
  if (!autoI) {
    // false 일때만 들어감(할당전에만)

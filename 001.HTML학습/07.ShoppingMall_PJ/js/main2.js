...
function loadFn() {
  ...
  for (let i = 0; i < 5; i++) {
    // 슬라이드 넣기
    slide.innerHTML += `
     <li data-seq="${i}"><img src="images/slide0${i + 1}.jpg" alt="slide"></li>
     `;
    // 블릿 넣기
    indic.innerHTML += `
    <li ${i === 0 ? 'class="on"' : ""}>
  ...
  function goSlide(evt, sts = true) {
    ...
    // 2-2. 왼쪽 버튼일 경우
      ...
      // 같은 left 값을 변경하기 때문에 코드 처리구역을 분리하여준다
      // 이때 사용되는 메서드는 setTimeout()!
      // 시간차는? 0을 줘도 코드를 분리하여 처리하므로 동시처리가 아니고 비동기처리하기 때문에 코드가 잘 작동한다
      setTimeout(() => {

...
Vue.component("list-comp", {
  ...
  template: `
      <div>
        ...
        v-on:mouseover="goMama({이름:'김고은',나이:'24살'})"
        alt="의류">
        <aside>
    ...
    `, // template //
  // 자식 컴포넌트에서 부모컴포넌트의 메서드를 바로 호출할 수 없다 따라서 자신의 메서드를 만들고
  // 그곳에서 호출방식에 따라 부모를 호출한다
  ...
  methods: {
    ...
    goPapa(txt) {
      ...
      // -> 생성 이벤트명이란 내가 만든 이벤트명으로 서브 컴포넌트 태그에 이벤트를 등록하여 호출하는 방식이다
...
new Vue({
  ...
  methods: {
    ...
    overMsg(pm) {
      ...
      console.log("오 마이 갓" + pm.이름 + "나이는" + pm.나이);
    },
  },
});

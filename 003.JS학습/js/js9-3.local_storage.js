...
/*************************************************************** 
    ...
    (1) 장점: 간단한 프론트엔드 데이터를 DB없이 테스트해보는데 탁월함
              ...
              이것을 거부하면 쓸 수 없는 JS 스토리지는 이런 제약이 없다.
              보안상 안정이 보장되므로 사용가능함
    (2) 단점: 데이터의 지속 보장이 없다!
...
mFn.qs("#mobtn").onclick = () => {
  ...
  // 4. 배열데이터에서 읽어온 옵션값 idx와 비교하여 데이터 업데이트
...
function updateItemList() {
  ...
  selBox.innerHTML =
    ...
    localData.map((v) => `<option value="${v.idx}">${v.idx}</option>`).join("");
} ////// updateItemList 함수 ///////

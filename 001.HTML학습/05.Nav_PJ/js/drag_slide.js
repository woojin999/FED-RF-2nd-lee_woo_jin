...
function slideFn(selEl) {
  ...
  ********************************************/
  ...
  function leftSlide(leftVal = "-330%") {
    ...
    // 함수전달변수를 leftVal="330%" 로 기본 입력값 처리하면
  ...
  ********************************************/
  ...
  // 마우스 다운시 z-index 대상 1로 만들때 다른 요소는 0
  // console.log(slider);
  ...
  console.log("기준값 * 110%:", valFirst);
  console.log("기준값 * 90% :", valSecond);
  // left위치값 최초셋업
  ...
  // 최초 위치 셋팅값으로 프리셋팅
  let lastX = leftVal;
  // -> 중첩된 최종위치가 처음에는 계산되지 않았으므로 출발위치인 0값으로 초기값을 넣어준다
  ...
  const dMove = (e) => {
    ...
    if (dragSts) {
      ...
      // - 모바일용 터치 스크린 터치위치는 touches[0].screenX, touches[0].screenY
      ...
      // 2. 움직일 위치 결과값
      // 첫번째 위치 포인트 - 움직일때 위치 포인트
      // firstX - moveX
      // firstY - moveY
      resultX = moveX - firstX;
      // 순수하게 움직인 거리 계산
      // 움직인 위치 - 첫번째 위치 순으로 빼준 이유는?
      // ->> top, left위치 이동 양수 음수차를 고려한 순서임

      // 3. 이동차를 구한 resultX,resultY값을 대상 위치값적용
      // 대상 : 드래그 요소 dtg
      dtg.style.left = resultX + lastX + "px";
      // 처음엔 lastX,lastY값이 0으로 들어오기
      // 두번째부터는 mouseup 이벤트 발생부터 저장된 최종 이동위치값이 더해진다
    ...
    // 드래그 중일때는 주먹손(grabbing), 드래그 아닐 때 편손(grab)
  ...
  // -> 왜 필요? 이동후 결과 위치를 저장하여 다음 드래그 이동시 그 결과를 중첩하여 반영하기 위해 필요함
  const lastPoint = () => {
  ...
  mFn.addEvt(dtg, "mousedown", (e) => {
    ...
    // 첫번째 위치포인트 셋팅
    firstPoint(e);
    // 단독할당되지 않고 내부 함수호출로 연결되어 있으므로 이벤트 전달을 토스 해줘야 한다(전달변수 e)
    dtg.style.cursor = "grabbing";

    // z-index 전역변수(zNum) 숫자를 1씩 높이기
    // dtg.style.zIndex = ++zNum;
    // console.log("다운", dragSts);
  }); ///// mousedown /////////

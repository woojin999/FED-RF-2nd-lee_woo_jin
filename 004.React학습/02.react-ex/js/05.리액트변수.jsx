...
-> current를 써야하는 이유는 useRef컴포넌트 객체에서 이 변수를 관리해주고 값을 유지 업데이트 하는데 사용하는 속성으로 current를 사용하도록 지정함
...
// (2) 단, 변경되지 않는 데이터 const 변수나 공유 함수들은 
// 컴포넌트 바깥에 코딩할수 있다
let what = "난뭐임?";
...
function MainComp() {
  ...
  // -> 의미: 실제 돔에 반영할 때 변경값이 업데이트 되는 시점
  const [stsName, setStsName] = React.useState("공유");

  // [2] 컴포넌트 내부 일반변수
  // -> 컴포넌트가 리랜더링 될때 다시 초기화 됨
  let varName = "김수현";

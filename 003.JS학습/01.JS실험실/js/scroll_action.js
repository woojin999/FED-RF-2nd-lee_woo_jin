...
/**************************************************** 
  ...
  3. 스크롤 등장 대상요소의 보이는 화면에서의 top값
    ...
    get 가져와라
    Bounding 경계선 (->바운딩 박스 -경계선을 가지는 직사각형박스)
    client 보이는 화면
    Rect 사각형
    -> BoundingClientRect -> 보이는 화면 사각형 경계선으로 부터의 거리를 리턴해주는 메서드
    -> 상단으로부터의 거리는 top속성
    -> 왼쪽으로 부터의 거리는 left속성
    공통적으로 경계선아래쪽은 양수, 윗쪽은 음수
...
function showLetters() {
  ...
  for (let x of myText) {
    ...
    if (x === " ") {
      ...
      // 글자일 경우 span태그 랩핑처리
      hcode += `<span style="transition-delay: ${seqNum * 0.1}s"
        >${x}</span>`;
    } /// if else //
  ...
  // 이것은 스크롤바 이동위치가 해당요소가 화면 맨위에 걸린 상태와 같음
  // 그러므로 화면 중간에 위치할때의 값은 화면높이값의 절반을 빼주면 된다
...
////////////////////////////
//////// 떨어지는 여자 구현하기 /////////////
// 기본 원리 : 스크롤 이동에 따른 화면 높이값 범위안에서 떨어지는 여자 이미지가 아래쪽으로 이동애니함
// 비례식을 세운다
// 스크롤 한계값 : 윈도우 높이 = 스크롤 이동값 : 이미지이동값
// 이미지 이동값 = 윈도우 높이 * 스크롤 이동값 / 스크롤한계값

// 0. 변수값 셋팅하기
// (1) 스크롤 한계값 : 전체document높이 - 화면 높이
// 전체document높이

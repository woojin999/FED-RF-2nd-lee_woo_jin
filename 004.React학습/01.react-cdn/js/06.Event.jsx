...
/************************************************************* 
    ...
    5. 리액트에 속성형태로 등록하는 이벤트는 html=요소에 등록된 이벤트 속성과 달리
    JS 이벤트 리스너를 통한 이벤트 객체에 등록되므로 html 태그상 이벤트 등록속성이 보이지 않는다

*************************************************************/

// 전체 이벤트 적용할 컴포넌트 구성하기 ////////
function EventShow() {
  // 컴포넌트 리턴 코드 위에서 이벤트 처리를 위한 함수를 만들어서 사용할수 있다
  // 지역함수로 사용되는 것
  ...
  const showAladin = () => {
    ...
    // 컴포넌트 호출시 전달변수를 셋팅하여 보내야 하는데
    // 만약 전달변수이름이 잘못되었거나 보내주지 않으면 컴포넌트에서 에러가 나지 않고
    // 해당 항목을 제외하여 표시하지 않는 특징 있음
  ...
  return (
    <React.Fragment>
      <div id="tbox" style={{ textAlign: "center" }}>
        {/* 스타일 인라인 적용시 바깥중괄호는 표현식 내부 중괄호는 객체형식의 스타일 설정임 */}
        <MakeImg

...
function showMenu() {
    ...
    // HTML 컬렉션 수집시 요소가 없으면 null값처리함 null도 데이터형 if문에서 false처리됨
    // null 의 뜻은 '빈값'
    // 호출확인
    ...
    if (smenu) { // null이 아니면 true처리됨
        ...
        // smenu의 높이값이 0이 맞으면 높이값 hval 아니면 0값 적용
        ...
        // gnb 상위 li를 모두 순회한다 
        gnbList.forEach(ele=>{
            // isSameNode() 메서드 : 순회중 같은 노드(요소)인지 판별해야주는 기능을 가짐(같으면 true)
            ...
            if (!isSame) { // !(not연산자)로 false일때 true변경
